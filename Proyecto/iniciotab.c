...
GtkWidget *AddButton(GtkWidget *theBox, const gchar *buttonText,
  ...
  gtk_signal_connect(GTK_OBJECT(button),"clicked",
		     GTK_SIGNAL_FUNC(CallBackFunction),EntryBox);
...
void Nuevo(GtkWidget *window, gpointer data){
  ...
  g_signal_connect_swapped (G_OBJECT (button), "clicked",
			    G_CALLBACK (gtk_widget_hide), G_OBJECT (windownombres));
  gtk_signal_connect(GTK_OBJECT(button),"clicked",
		     GTK_SIGNAL_FUNC(NOMBRES),Inicio);
  gtk_widget_show(button);
...
void Cargar(GtkWidget *window, gpointer data){
  ...
  button = AddButton1(windownombres,verticalbox,"Continuar",
		      CARGAR,Inicio);
...
void quick_message (gchar *message, GtkWidget *parent) {
  ...
  gtk_signal_connect(GTK_OBJECT(button),"clicked",
		     GTK_SIGNAL_FUNC(gtk_widget_hide_all),window);
  ...
  gtk_signal_connect(GTK_OBJECT(window),"destroy",
		       GTK_SIGNAL_FUNC(gtk_widget_show),parent);
