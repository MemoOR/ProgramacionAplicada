...
GtkWidget *create_pad(gpointer data) {
  ...
  pix=gdk_pixbuf_new_from_file("white.png",NULL);
  pix=gdk_pixbuf_scale_simple(pix,30,30,GDK_INTERP_BILINEAR);
  GtkWidget *img_w = gtk_image_new_from_pixbuf(pix);
  ...
  for (int i = 0; i < 20; i++) {
    ...
    for(int j = 0; j < 20; j++){
      ...
      gtk_button_set_image (GTK_BUTTON (button), img_w);
